...
The assessment suite includes **292 security checks** across 15 collector families (Entra, CA Evaluator, EntApp, EXO, DNS, Defender, Compliance, Stryker Readiness, Intune, SharePoint, Teams, Power BI, Forms, Purview Retention, Backup), each mapped to one or more compliance frameworks.

## Control Registry

Framework mappings are defined in `controls/registry.json`, which contains **292 control entries** — the M365-scoped subset of the upstream CheckID registry plus local extension checks. (CheckID upstream also carries Windows-endpoint `WIN-*` and Azure-subscription `AZ-*` checks; the sync workflow filters those out via `controls/sync-scope.json` because no collector in this module can emit them.) Each entry specifies the check ID, description, and mappings to all applicable frameworks.
